...
const CategoriesList = () => {
    ...
    const handleType = (e) => {
        ...
        console.log(type)
        setFilteredptions(user.categories.filter(item => item.transactionType === type))
    }
            ...
            {filteredOptions.map(item =>
                <li key={item._id} className='collection-item row'>
                    <div className="text col s9">
                        <span className="col s3">{item.name.charAt(0).toUpperCase() + item.name.slice(1)}</span>
                        <span className="col s3">{item.transactionType.charAt(0).toUpperCase() + item.transactionType.slice(1)}</span>
                        <span className='dot' style={{ backgroundColor: `${item.color}` }}></span>
                    </div>
                    <div className="icons">
                        <a href="#edit-category-modal"
                            className='secondary-content modal-trigger'
                            onClick={() => getCategory(item)}
                        >
                            <i className='material-icons green-text'>edit</i>
                        </a>
                        <a href='#!' className='secondary-content' onClick={() => handleDelete(item._id)}>
    ...
    return (
        <>
            ...
            {filteredOptions.length === 0 ? <p>No categories found</p> : showCategories}
